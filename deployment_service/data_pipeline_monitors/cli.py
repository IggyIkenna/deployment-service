...
def _list_running_vms() -> list[tuple[str, str]] | None:
    """Return ``(vm_name, zone)`` for every RUNNING VM, or ``None`` if the census is UNAVAILABLE.
    ...
    isolated: an API error or timeout returns ``None`` (NOT an empty list) so
    callers can distinguish "the census is unavailable" from "the census is
    healthy and genuinely empty".  That distinction is load-bearing for the
    ``reap_stale()`` wiring below — a failure-collapsed empty list was passed as
    ``running_vm_names={}``, and ``_reap_reason`` then classified EVERY
    stale-heartbeat active entry as ``vm_not_running``, falsely reaping a
    genuinely-RUNNING VM
    (cefi_content_apply_memory_freeze_recurs_post_fix_and_registry_false_reap_2026_07_31.md
    Finding 3).  The sweep treats ``None`` identically to ``[]`` for its own
    no-op purposes, but passes ``running_vm_names=None`` so the reaper falls back
    to heartbeat-age-only (never over-reaps on an unavailable census).
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                ...
                "returning None so sentinel write still fires; stuck gRPC thread abandoned",
    ...
    with run_lifecycle(service_name="dp-fleet-monitor"):
        ...
        if mode == "exit-code":
            ...
            while True:
                ...
                # ``None`` (census unavailable) behaves like [] for the sweep's own
                # no-op purposes, but the reap_stale wiring below must NOT turn it
                # into an empty ``running_vm_names`` set (Finding 3 false-reap).
                running = [vm for vm in (all_running_vms or []) if _is_data_vm(vm[0])]
                ...
                if not dry_run:
                    ...
                    try:
                        # When the census is unavailable (None), pass running_vm_names=None
                        # so _reap_reason falls back to heartbeat-age-only classification —
                        # an empty set here would read as "no VMs running" and over-reap
                        # every stale-heartbeat entry as vm_not_running (Finding 3, the
                        # 2026-07-31T05:46:53Z false reap of a still-RUNNING VM).
                        reaped = DeploymentsRegistry().reap_stale(
                            running_vm_names=(
                                {vm_name for vm_name, _zone in all_running_vms} if all_running_vms is not None else None
                            )
                        )
                        if reaped:
                            logger.info(
            ...
            running = [vm for vm in (_list_running_vms() or []) if _is_data_vm(vm[0])]
            ...
            # DP-VM-013: generic Cloud Run Job per-execution failure for all jobs in
            # cloud_run_job_registry.CLOUD_RUN_JOBS except manifest-consolidator-* (above).
            # infra_health_audit_alert_coverage_gaps_2026_08_07.md finding (A).
            cloud_run_job_failure_watcher.check_cloud_run_job_executions(
