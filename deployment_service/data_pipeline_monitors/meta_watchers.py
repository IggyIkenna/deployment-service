...
from deployment_service.data_pipeline_monitors.attempted_failed_staleness import (
    ...
    stale_days_since,
    trailing_attempted_failed_count,
)
...
ATTEMPTED_FAILED_TRAILING_WINDOW_DAYS = 7  # look-back for the HIGH verdict (not the lifetime total)
# STATIC_BACKLOG_STALE_DAYS_THRESHOLD: labeling-only, see attempted_failed_staleness.py.
...
class AttemptedFailedCell:
    ...
    trailing_attempted_failed: int = 0  # attempted_failed within ATTEMPTED_FAILED_TRAILING_WINDOW_DAYS (drives HIGH)
...
def _read_attempted_failed_cells(
    ...
    *,
    ...
    for data_type in sorted(data_type_col.unique()):
        ...
        trailing_af = trailing_attempted_failed_count(
            failed_attempted_at,
            window_days=ATTEMPTED_FAILED_TRAILING_WINDOW_DAYS,
            lifetime_count=attempted_failed,
        )
        high = trailing_af >= ATTEMPTED_FAILED_ABS_THRESHOLD or (
            trailing_af >= MIN_ATTEMPTED_FAILED_FOR_RATIO and ratio >= ATTEMPTED_FAILED_RATIO_THRESHOLD
        )
        cells.append(
            AttemptedFailedCell(
...
def check_high_attempted_failed(
    *,
    ...
    A cell is HIGH when its **trailing** ``attempted_failed`` count (rows whose
    ``attempted_at`` falls within ``ATTEMPTED_FAILED_TRAILING_WINDOW_DAYS`` of now)
    crosses ``ATTEMPTED_FAILED_ABS_THRESHOLD`` OR (trailing count >=
    ``MIN_ATTEMPTED_FAILED_FOR_RATIO`` AND ratio >= ``ATTEMPTED_FAILED_RATIO_THRESHOLD``).
    Using a trailing window — not the lifetime total — means a cell whose failures are
    dominated by already-fixed historical incidents stops paging once those rows age out
    of the window (closes the cefi/liquidations always-on-CRITICAL class, 2026-08-07).
    Fall-safe: when ``attempted_at`` is absent for all rows, the lifetime count is used.
    When ``miss_tracker`` is
    ...
    for target in targets:
        ...
        for cell in cells:
            ...
            summary = (
                ...
                f"data_type={cell.data_type}: "
                f"{cell.trailing_attempted_failed}/{cell.attempted_failed} "
                f"attempted_failed cells in last {ATTEMPTED_FAILED_TRAILING_WINDOW_DAYS}d/lifetime "
