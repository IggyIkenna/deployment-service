...
async def get_experiment(experiment_id: str) -> ExperimentDetail:
    ...
    try:
        ...
        metrics_data: object = json.loads(raw_bytes.decode("utf-8"))
        metrics: dict[str, object] = metrics_data if isinstance(metrics_data, dict) else {}
...
async def list_models(limit: int = 100) -> list[ModelSummary]:
    ...
    try:
        ...
        try:
            ...
            manifest_data: object = json.loads(manifest_bytes.decode("utf-8"))
            manifest: dict[str, object] = manifest_data if isinstance(manifest_data, dict) else {}
...
async def get_model_metadata(model_id: str, training_period: str | None = None) -> ModelMetadataResponse:
    ...
    try:
        ...
        raw_bytes = storage_client.download_bytes(bucket=bucket, blob_path=gcs_path)
        metadata_data: object = json.loads(raw_bytes.decode("utf-8"))
        metadata: dict[str, object] = metadata_data if isinstance(metadata_data, dict) else {}
...
def _resolve_latest_period(
    ...
    try:
        ...
        manifest_data: object = json.loads(manifest_bytes.decode("utf-8"))  # pyright: ignore[reportUnknownMemberType]
        manifest: dict[str, object] = manifest_data if isinstance(manifest_data, dict) else {}
        models_dict = manifest.get("models", {})
