...
class StorageClient:
    ...
    def __init__(self, project_id: str | None = None, provider: str = "gcs"):
        ...
        self._mock_mode: bool = bool(cast(bool, _config.cloud_mock_mode))
    ...
    def _list_gcs_files(self, bucket_name: str, prefix: str, pattern: str = "*", max_results: int = 1000) -> list[str]:
        ...
        try:
            ...
            return [f"gs://{bucket_name}/{blob.name}" for blob in blobs if self._matches_pattern(blob.name, pattern)]
        except (OSError, PermissionError):
            return []
    ...
    def _parse_cloud_path(self, cloud_path: str) -> tuple[str, str, str]:
        ...
        if cloud_path.startswith("gs://"):
            ...
            raise ValueError(f"Invalid cloud path: {cloud_path}. Must start with 'gs://' or 's3://'")
        parts = path.split("/", 1)
        bucket_name = parts[0]
