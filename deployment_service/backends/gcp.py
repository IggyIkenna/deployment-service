...
class CloudRunBackend(ComputeBackend):
    ...
    def _get_template_env(self) -> list[run_v2.EnvVar] | None:
        ...
        try:
            ...
            execution_template: object = getattr(job_template, "template", None) if job_template else None
            containers: object = getattr(execution_template, "containers", None) if execution_template else None
    ...
    def _merge_env(self, template_env: list[run_v2.EnvVar], overrides: dict[str, str]) -> list[run_v2.EnvVar]:
        ...
        for env in template_env:
            ...
            if name in overrides:
                ...
                if value_source:
                    logger.warning(
                        "[CLOUD_RUN_ENV] Overriding secret-backed env var '%s' for execution overrides",
                        name,
                    )
