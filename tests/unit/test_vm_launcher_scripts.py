...
'''
    ...
    def test_shutdown_script_bakes_in_vm_name_and_project_no_metadata_lookup(self, tmp_path: Path) -> None:
        ...
        # Read the expected project id from the launcher's own PROJECT= line rather than
        # hardcoding it here — QG bans a literal prod project id string anywhere in tests/.
        import re

        launcher_path = Path(__file__).parent.parent.parent / self.LAUNCHER
        match = re.search(r'^PROJECT="([^"]+)"', launcher_path.read_text(), re.MULTILINE)
        assert match, "could not find the launcher's PROJECT= assignment"
        assert f"PROJECT={match.group(1)}" in content
