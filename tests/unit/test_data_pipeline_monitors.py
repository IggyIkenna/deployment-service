...
def test_high_attempted_failed_pages_on_abs_threshold(monkeypatch):
    ...
    # Failures must be within the trailing window to cross the threshold.
...
def test_high_attempted_failed_pages_on_ratio_threshold(monkeypatch):
    ...
    # Failures must be within the trailing window to cross the ratio threshold.
...
def test_known_dead_cell_suppressed_despite_crossing_threshold(monkeypatch):
    # The registered CBOE ohlcv_15m cell: all failures pre-date the narrowing date AND
    # are outside the 14-day trailing window → cell is not high (trailing count=0) AND
    # known_dead=True. The known-dead flag provides defense-in-depth for cells with recent
    # narrowing dates whose failures may still fall within the window but before narrowing.
    n_failed = meta_watchers.ATTEMPTED_FAILED_ABS_THRESHOLD + 5
    ...
    assert not cell.high  # trailing window excludes stale failures → below threshold
    assert cell.known_dead  # and the registered known-dead flag also holds
...
def test_known_dead_cell_with_new_activity_still_pages(monkeypatch):
    # Same registered cell, but rows AFTER the narrowing date AND within the trailing
    # window cross the abs threshold → known_dead clears (new activity post-narrowing)
    # AND trailing count is high → cell pages.
    n_old = meta_watchers.ATTEMPTED_FAILED_ABS_THRESHOLD + 4  # old (outside trailing window)
    n_fresh = meta_watchers.ATTEMPTED_FAILED_ABS_THRESHOLD + 5  # recent (within window, post-narrowing)
    fresh_ts = (datetime.now(UTC) - timedelta(days=5)).strftime("%Y-%m-%dT%H:%M:%SZ")
    ...
    cells = meta_watchers.check_high_attempted_failed(storage_client=storage, targets=[_af_target(label="tradfi")])
    cell = next(c for c in cells if c.data_type == "ohlcv_15m")
    assert cell.high  # trailing window count crosses ABS_THRESHOLD
    assert not cell.known_dead  # fresh_ts > narrowed_at (2026-07-15) → not known dead
    assert any(e[0] == "DP_RUN_MOSTLY_EMPTY" and e[1] == "CRITICAL" for e in emitted)
...
def test_unregistered_cell_never_known_dead(monkeypatch):
    ...
    # Use a fresh timestamp so the trailing window count crosses the threshold.
    n_failed = meta_watchers.ATTEMPTED_FAILED_ABS_THRESHOLD + 5
    fresh_ts = (datetime.now(UTC) - timedelta(days=5)).strftime("%Y-%m-%dT%H:%M:%SZ")
