...
def test_high_attempted_failed_pages_on_abs_threshold(monkeypatch):
    ...
    # Fresh attempted_at so the trailing-window count reflects the full batch.
...
def test_high_attempted_failed_pages_on_ratio_threshold(monkeypatch):
    ...
    # Fresh attempted_at so trailing-window count == lifetime count for this assertion.
...
def test_known_dead_cell_suppressed_despite_crossing_threshold(monkeypatch):
    # The registered CBOE ohlcv_15m cell: high (crosses ABS_THRESHOLD) but is marked
    # known-dead → suppressed, never pages.  We monkeypatch is_known_dead so that the
    # trailing-window logic (which sees fresh rows) and the registry's narrowing-date
    # comparison (which requires old rows) don't conflict with each other.
    n_failed = meta_watchers.ATTEMPTED_FAILED_ABS_THRESHOLD + 5
    fresh_ts = datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
    rows = [("ohlcv_15m", "attempted_failed", fresh_ts)] * n_failed
    monkeypatch.setattr(meta_watchers, "is_known_dead", lambda ag, dt, *, max_attempted_at="": True)
    storage = FakeStorage({(_MD_BUCKET, _AVAIL_INDEX): (_index_parquet(rows), 0.0)})
    emitted = _capture_emits(monkeypatch)
...
def test_known_dead_cell_with_new_activity_still_pages(monkeypatch):
    # Same registered cell, but rows carry attempted_at AFTER the narrowing date AND
    # within the 14-day trailing window → known_dead clears, trailing count is high,
    # cell must page.  All rows are fresh so that trailing_attempted_failed >= ABS_THRESHOLD.
    n_failed = meta_watchers.ATTEMPTED_FAILED_ABS_THRESHOLD + 5
    fresh_ts = datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
    rows = [("ohlcv_15m", "attempted_failed", fresh_ts)] * n_failed
    storage = FakeStorage({(_MD_BUCKET, _AVAIL_INDEX): (_index_parquet(rows), 0.0)})
...
def test_unregistered_cell_never_known_dead(monkeypatch):
    ...
    # Fresh timestamps so trailing-window count reflects the full batch.
    n_failed = meta_watchers.ATTEMPTED_FAILED_ABS_THRESHOLD + 5
    fresh_ts = datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
...
def test_high_attempted_failed_consecutive_miss_suppresses_first_then_pages_second(monkeypatch):
    ...
    # Fresh attempted_at so trailing-window count reflects the full batch.
    n_failed = meta_watchers.ATTEMPTED_FAILED_ABS_THRESHOLD + 5
    fresh_ts = datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
    rows = [("trades", "captured")] * 100 + [("trades", "attempted_failed", fresh_ts)] * n_failed
    storage = FakeStorage({(_MD_BUCKET, _AVAIL_INDEX): (_index_parquet(rows), 0.0)})
