"""
Unit tests for turbo data status API - Analytics Operations.

Tests completion calculations, metrics consistency, and dimension weighting:
- Completion percentage calculation formulas
- Dimension-weighted venue completion calculation
- Metric consistency between different response fields
- Category aggregation using dimension-weighted values
"""
...
class TestCompletionCalculation:
    ...
    def test_completion_pct_formula(self, sample_completion_calculation_data):
        """Test that completion percentage is calculated correctly."""
        # Formula: (dates_found / dates_expected) * 100
        test_cases = sample_completion_calculation_data["basic_cases"]

        for case in test_cases:
            found = case["dates_found"]
            expected = case["dates_expected"]
            expected_pct = case["expected_pct"]

            if expected > 0:
                pct = round((found / expected) * 100, 1)
                assert pct == expected_pct, f"Expected {expected_pct}%, got {pct}%"

    def test_zero_expected_dates(self, sample_completion_calculation_data):
        """Test handling of zero expected dates (avoid division by zero)."""
        edge_cases = sample_completion_calculation_data["edge_cases"]
...
class TestDimensionWeightedCompletion:
    ...
    def test_venue_completion_reflects_all_data_types(self, sample_dimension_weighted_data):
        ...
        """
        venue_data = sample_dimension_weighted_data["venue_with_data_types"]["UNISWAP_V3-ETHEREUM"]
        ...
        assert new_pct == expected_completion
    ...
    def test_category_aggregation_uses_dimension_weighted_values(self, sample_dimension_weighted_data):
        ...
        expected_completion = category_data["expected_category_completion"]
