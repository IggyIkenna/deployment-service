#!/usr/bin/env bash
# Launch 1 GCE VM for perpetual funding rate backfill
#
# Fetches Hyperliquid perpetual funding rates from their public S3 archive
...
TARBALL_NAME="perp_funding_codebase.tar.gz"
...
echo "  Data:   Hyperliquid perpetual funding rates (public S3 archive)"
...
exec > >(tee /var/log/perp-funding.log) 2>&1
...
(while true; do
  sleep 60
  gsutil -q cp /var/log/perp-funding.log \
...
echo ""
echo "=== Starting perp funding rate backfill ==="
echo "  Range: ${START_DATE} -> ${END_DATE}"
date

cd market-tick-data-service
python3 -m market_tick_data_service \
  --operation collect-perp-funding \
  --mode batch \
...
echo ""
echo "SSH + tail log:"
echo "  gcloud compute ssh ${VM_NAME} --zone=${ZONE} --project=${PROJECT_ID} -- tail -f /var/log/perp-funding.log"
echo ""
echo "Stream logs from GCS:"
echo "  gsutil cat ${GCS_STAGING}/logs/${VM_NAME}_final.log | tail -100"
echo ""
echo "Check results:"
echo "  gsutil ls ${TICK_BUCKET}/raw_tick_data/by_date/ | grep perp | head -20"
echo "============================================================"
