...
#
# Covers the Jito mainnet window: 2022-08-01 → today.
# Jito stake pool launched August 2022; earlier dates emit record_empty
# with EXPECTED_NOT_YET_LAUNCHED.
#
# Purpose: carry_staked_basis Solana leg requires historical jitoSOL staking APY.
# The LstRatesHandler._LST_TOKENS includes jitoSOL alongside EVM tokens; this
# launcher provides a dedicated Jito-scoped entry point with the correct start
# date and watchdog prefix.
#
# GCS BACKFILL RULE (MANDATORY): ~4 years of data → operator [ack] REQUIRED.
...
#
# Usage:
#   bash launch-jito-solana-backfill-vm.sh                       # 2022-08-01 → today
#   bash launch-jito-solana-backfill-vm.sh 2022-08-01 2026-05-14 # explicit window
#   bash launch-jito-solana-backfill-vm.sh --force ...           # bypass singleton
#   bash launch-jito-solana-backfill-vm.sh --env staging ...     # staging env
#
# Cost: e2-standard-4 + 50GB. ~4 years × 13 EVM + 2 Solana tokens. Expect 2-4h.
#
# Singleton lock: refuses to launch if another jito-solana-backfill-* VM is
# RUNNING in the zone to prevent Alchemy compute-unit budget burn.
#
...
if [[ ${#POSITIONAL[@]} -eq 2 ]]; then
    ...
    # Default: Jito mainnet launch → today.
...
Defaults to the Jito mainnet window (2022-08-01 → today).
...
GCS BACKFILL RULE: ~4 years of data — requires operator [ack] before launch.
EOF
    exit 1
fi
...
if ! $FORCE; then
    EXISTING="$(gcloud compute instances list \
        --filter='name~"^jito-solana-backfill-" AND status=RUNNING' \
        --zones="$ZONE" \
        --format='value(name)' 2>/dev/null | head -1)"
    if [[ -n "$EXISTING" ]]; then
        cat >&2 <<EOF
ERROR: Jito-Solana backfill VM already running in $ZONE: $EXISTING
...
VM_NAME="jito-solana-backfill-${RUN_TS}"

# SPOT by default; --on-demand / ON_DEMAND=true forces standard provisioning.
PROVISIONING_FLAGS="--provisioning-model=SPOT --instance-termination-action=DELETE --no-restart-on-failure"
if $ON_DEMAND; then PROVISIONING_FLAGS=""; fi

echo "Launching $VM_NAME: JITO-SOLANA lst_rates backfill ${START_DATE}..${END_DATE} [$([[ -n "$PROVISIONING_FLAGS" ]] && echo SPOT || echo on-demand)]"
...
if [[ "${DRY_RUN:-false}" == "true" ]]; then
  ...
  gcloud compute instances create "$VM_NAME" \
      ...
      --labels=purpose=jito-solana-backfill,env="${DEPLOYMENT_ENV}",run-ts="${RUN_TS}",managed-by=deployment-service
fi
