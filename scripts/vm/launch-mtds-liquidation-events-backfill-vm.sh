...
# Lifecycle: permanent
# Delete-when: NA
# Launch GCE VM for Liquidation events (per-protocol liquidation event stream via The Graph)
#
...
#
# Usage:
#   bash launch-mtds-liquidation-events-backfill-vm.sh
#   bash launch-mtds-liquidation-events-backfill-vm.sh --start 2024-01-01 --end 2026-01-01
#   bash launch-mtds-liquidation-events-backfill-vm.sh --dry-run
#   bash launch-mtds-liquidation-events-backfill-vm.sh --env staging
set -euo pipefail

PROJECT_ID="${PROJECT_ID:-central-element-323112}"
ZONE="${ZONE:-asia-northeast1-c}"
MACHINE_TYPE="${MACHINE_TYPE:-e2-highmem-8}"  # 64GB: defi manifest _index merge peaks ~few GB (see manifest_index_read_oom issue)
DRY_RUN=false
...
VM_NAME="${VM_NAME:-mtds-liquidation-events-backfill}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/lib/launcher_common.sh"

echo "============================================================"
echo "liquidation-events Backfill VM Launcher (Pattern A)"
echo "  VM:        ${VM_NAME}"
...
if ! $FORCE; then
  EXISTING="$(gcloud compute instances list \
    --filter="name~\"^mtds-liquidation-events-\" AND status=RUNNING" \
    --zones="${ZONE}" \
    --project="${PROJECT_ID}" \
    --format='value(name)' 2>/dev/null | head -1 || true)"
  if [[ -n "$EXISTING" ]]; then
    echo "WARN: liquidation-events VM already running: ${EXISTING}" >&2
...
if $DRY_RUN; then
  ...
  echo "  VM_TASK=defi-backfill  VM_OPERATION=collect-liquidation-events"
  exit 0
fi
...
METADATA="${METADATA},VM_OPERATION=collect-liquidation-events"
...
gcloud compute instances create "${VM_NAME}" \
  ...
  --labels="purpose=mtds-liquidation-events-backfill,env=${DEPLOYMENT_ENV}",managed-by=deployment-service \
  --metadata="${METADATA}"
