...
echo ""
echo "Post-backfill manifest merge (additive, preserves rows outside the prefix;"
echo "one per features bucket — use the writer's own object-key prefix,"
echo "e.g. delta_one/cross_instrument→'delta_one/by_date', sports→'sports_features/n',"
echo "volatility→'volatility/n', calendar→'calendar/<subtype>/n', onchain varies):"
...
echo "      prefix='<per-family prefix, see above>')\""
