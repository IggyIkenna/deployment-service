...
echo "sports is 'sports_features/by_date', not 'features/by_date'; ADDITIVE merge only —"
echo "rebuild_manifest_from_canonical_paths() wholesale-replaces a shared bucket's WHOLE manifest"
echo "index; features-{ag} is a Group-B folded bucket co-locating multiple families under ONE"
echo "index, so a wholesale rebuild would delete every OTHER family's rows — see"
