...
# ── Fail loud, immediately, if gcs_upload_via_adc.py's import target isn't available ──
# create-code-tarballs.sh resolves GCS_UPLOAD_PY as "${DS_ROOT}/.venv/bin/python", falling back to
# bare `python3` when no venv exists. The job's image (deployment-service:latest, see
# code_tarball_refresh_scheduler.tf) already has `deployment_service` installed as a real package —
# this is just a fast pre-flight check so a future image regression (missing the package again,
# per code_tarball_refresh_job_silently_failing_since_2026_07_30_2026_08_01.md's silent-failure
# incident) fails immediately and loudly instead of N per-repo uploads crashing one at a time.
python3 -c "import deployment_service" >/dev/null 2>&1 \
    || { log "ERROR: python3 cannot import deployment_service -- the job image is missing it (or lost it). Aborting before any upload attempt."; exit 1; }
