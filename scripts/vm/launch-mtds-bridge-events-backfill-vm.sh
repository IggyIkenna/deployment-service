...
# Lifecycle: permanent
# Delete-when: NA
# Launch GCE VM for bridge-events historical backfill (ACROSS + STARGATE transfer events
# via Alchemy eth_getLogs — genesis ACROSS 2021-11-11, STARGATE 2022-03-17).
#
# Pattern A (canonical tarball) — startup-script-url=gs://.../vm/setup-data-pipeline-vm.sh
#
# Usage:
#   bash launch-mtds-bridge-events-backfill-vm.sh
#   bash launch-mtds-bridge-events-backfill-vm.sh --start 2021-11-11 --end 2026-01-01
#   bash launch-mtds-bridge-events-backfill-vm.sh --dry-run
#   bash launch-mtds-bridge-events-backfill-vm.sh --env staging
set -euo pipefail

PROJECT_ID="${PROJECT_ID:-central-element-323112}"
ZONE="${ZONE:-asia-northeast1-c}"
MACHINE_TYPE="${MACHINE_TYPE:-e2-highmem-8}"  # 64GB: defi manifest _index merge peaks ~few GB
DRY_RUN=false
START_DATE="${START_DATE:-2021-11-11}"
END_DATE="${END_DATE:-$(date +%Y-%m-%d)}"
FORCE=false
DEPLOYMENT_ENV="${DEPLOYMENT_ENV:-prod}"
# Idempotent backfill defaults to SPOT (~60-91% cheaper).
...
VM_NAME="${VM_NAME:-mtds-bridge-events-backfill}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/lib/launcher_common.sh"

echo "============================================================"
echo "bridge-events Backfill VM Launcher (Pattern A)"
echo "  VM:        ${VM_NAME}"
...
if ! $FORCE; then
  EXISTING="$(gcloud compute instances list \
    --filter="name~\"^mtds-bridge-events-\" AND status=RUNNING" \
    --zones="${ZONE}" \
    --project="${PROJECT_ID}" \
    --format='value(name)' 2>/dev/null | head -1 || true)"
  if [[ -n "$EXISTING" ]]; then
    echo "WARN: bridge-events VM already running: ${EXISTING}" >&2
...
if $DRY_RUN; then
  ...
  echo "  VM_TASK=defi-backfill  VM_OPERATION=collect-bridge-events"
  exit 0
fi
...
METADATA="${METADATA},VM_OPERATION=collect-bridge-events"
...
gcloud compute instances create "${VM_NAME}" \
  ...
  --labels="purpose=mtds-bridge-events-backfill,env=${DEPLOYMENT_ENV}",managed-by=deployment-service \
  --metadata="${METADATA}"
