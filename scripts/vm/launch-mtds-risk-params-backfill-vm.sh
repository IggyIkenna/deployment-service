...
# Lifecycle: permanent
# Delete-when: NA
# Launch GCE VM for risk_params backfill (per-lending-market reserve config: ltv/liq_threshold/liq_bonus/reserve_factor — Aave/Morpho/Compound/Spark/Fluid/Kamino via The Graph)
#
...
#
# Usage:
#   bash launch-mtds-risk-params-backfill-vm.sh
#   bash launch-mtds-risk-params-backfill-vm.sh --start 2024-01-01 --end 2026-01-01
#   bash launch-mtds-risk-params-backfill-vm.sh --dry-run
#   bash launch-mtds-risk-params-backfill-vm.sh --env staging
set -euo pipefail

PROJECT_ID="${PROJECT_ID:-central-element-323112}"
ZONE="${ZONE:-asia-northeast1-c}"
MACHINE_TYPE="${MACHINE_TYPE:-e2-highmem-8}"  # 64GB: defi manifest _index merge peaks ~few GB (manifest_index_read_oom issue)
DRY_RUN=false
...
VM_NAME="${VM_NAME:-mtds-risk-params-backfill}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/lib/launcher_common.sh"

echo "============================================================"
echo "risk-params Backfill VM Launcher (Pattern A)"
echo "  VM:        ${VM_NAME}"
...
if ! $FORCE; then
  EXISTING="$(gcloud compute instances list \
    --filter="name~\"^mtds-risk-params-\" AND status=RUNNING" \
    --zones="${ZONE}" \
    --project="${PROJECT_ID}" \
    --format='value(name)' 2>/dev/null | head -1 || true)"
  if [[ -n "$EXISTING" ]]; then
    echo "WARN: risk-params VM already running: ${EXISTING}" >&2
...
if $DRY_RUN; then
  ...
  echo "  VM_TASK=defi-backfill  VM_OPERATION=collect-risk-params"
  exit 0
fi
...
METADATA="${METADATA},VM_OPERATION=collect-risk-params"
...
gcloud compute instances create "${VM_NAME}" \
  ...
  --labels="purpose=mtds-risk-params-backfill,env=${DEPLOYMENT_ENV}",managed-by=deployment-service \
  --metadata="${METADATA}"
