...
# Lifecycle: permanent
# Delete-when: NA
# Launch GCE VM for Flash loan events (Aave V3 flashloan stream via The Graph)
#
...
#
# Usage:
#   bash launch-mtds-flash-loan-events-backfill-vm.sh
#   bash launch-mtds-flash-loan-events-backfill-vm.sh --start 2024-01-01 --end 2026-01-01
#   bash launch-mtds-flash-loan-events-backfill-vm.sh --dry-run
#   bash launch-mtds-flash-loan-events-backfill-vm.sh --env staging
set -euo pipefail
...
VM_NAME="${VM_NAME:-mtds-flash-loan-events-backfill}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/lib/launcher_common.sh"

echo "============================================================"
echo "flash-loan-events Backfill VM Launcher (Pattern A)"
echo "  VM:        ${VM_NAME}"
...
if ! $FORCE; then
  EXISTING="$(gcloud compute instances list \
    --filter="name~\"^mtds-flash-loan-events-\" AND status=RUNNING" \
    --zones="${ZONE}" \
    --project="${PROJECT_ID}" \
    --format='value(name)' 2>/dev/null | head -1 || true)"
  if [[ -n "$EXISTING" ]]; then
    echo "WARN: flash-loan-events VM already running: ${EXISTING}" >&2
...
if $DRY_RUN; then
  ...
  echo "  VM_TASK=defi-backfill  VM_OPERATION=collect-flash-loan-events"
  exit 0
fi
...
METADATA="${METADATA},VM_OPERATION=collect-flash-loan-events"
...
gcloud compute instances create "${VM_NAME}" \
  ...
  --labels="purpose=mtds-flash-loan-events-backfill,env=${DEPLOYMENT_ENV}",managed-by=deployment-service \
  --metadata="${METADATA}"
