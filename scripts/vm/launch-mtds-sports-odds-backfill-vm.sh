...
# VM_ASSET_GROUP=SPORTS + VM_TIER for Odds API tier selection.
#
# Usage:
...
#   bash launch-mtds-sports-odds-backfill-vm.sh --tier 2 --start 2025-01-01 --end 2026-03-28
...
if $DRY_RUN; then
  ...
  echo "  VM_TASK=mtds-backfill  VM_ASSET_GROUP=SPORTS  VM_TIER=${TIER}"
  exit 0
fi
...
METADATA="${METADATA},VM_TIER=${TIER}"
