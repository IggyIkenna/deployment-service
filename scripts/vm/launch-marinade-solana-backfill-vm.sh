...
echo "Launching $VM_NAME: MARINADE-SOLANA lst_rates backfill ${START_DATE}..${END_DATE} [$([[ -n "$PROVISIONING_FLAGS" ]] && echo SPOT || echo on-demand)]"
...
if [[ "${DRY_RUN:-false}" == "true" ]]; then
  ...
  gcloud compute instances create "$VM_NAME" \
      ...
      --labels=purpose=marinade-backfill,env="${DEPLOYMENT_ENV}",run-ts="${RUN_TS}",managed-by=deployment-service
fi
