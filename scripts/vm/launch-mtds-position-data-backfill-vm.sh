...
# Lifecycle: permanent
# Delete-when: NA
# Launch GCE VM for Position data (Aave V3 user positions + Uniswap V3 LP positions via The Graph)
#
...
#
# Usage:
#   bash launch-mtds-position-data-backfill-vm.sh
#   bash launch-mtds-position-data-backfill-vm.sh --start 2024-01-01 --end 2026-01-01
#   bash launch-mtds-position-data-backfill-vm.sh --dry-run
#   bash launch-mtds-position-data-backfill-vm.sh --env staging
set -euo pipefail
...
VM_NAME="${VM_NAME:-mtds-position-data-backfill}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/lib/launcher_common.sh"

echo "============================================================"
echo "position-data Backfill VM Launcher (Pattern A)"
echo "  VM:        ${VM_NAME}"
...
if ! $FORCE; then
  EXISTING="$(gcloud compute instances list \
    --filter="name~\"^mtds-position-data-\" AND status=RUNNING" \
    --zones="${ZONE}" \
    --project="${PROJECT_ID}" \
    --format='value(name)' 2>/dev/null | head -1 || true)"
  if [[ -n "$EXISTING" ]]; then
    echo "WARN: position-data VM already running: ${EXISTING}" >&2
...
if $DRY_RUN; then
  ...
  echo "  VM_TASK=defi-backfill  VM_OPERATION=collect-position-data"
  exit 0
fi
...
METADATA="${METADATA},VM_OPERATION=collect-position-data"
...
gcloud compute instances create "${VM_NAME}" \
  ...
  --labels="purpose=mtds-position-data-backfill,env=${DEPLOYMENT_ENV}",managed-by=deployment-service \
  --metadata="${METADATA}"
