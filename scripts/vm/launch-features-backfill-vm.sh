...
echo ""
echo "Post-backfill manifest rebuild (one per features bucket, ADDITIVE merge only —"
echo "rebuild_manifest_from_canonical_paths() wholesale-replaces a shared bucket's WHOLE manifest"
echo "index; features-{ag} is a Group-B folded bucket co-locating delta_one/volatility/onchain/"
echo "xinstrument/mtf under ONE index, so a wholesale rebuild would delete every OTHER family's rows —"
echo "see rebuild_manifest_from_canonical_paths_prefix_scoped_wipe_2026_07_27.md. Resolve the bucket"
echo "name + the real per-family object-key prefix (NOT 'features/by_date' — see"
echo "codex/05-infrastructure/bucket-isolation-model.md) before running this):"
...
echo "      service_name='features-${SERVICE_SHORT}-service', prefix='<real-family-prefix>/by_date')\""
