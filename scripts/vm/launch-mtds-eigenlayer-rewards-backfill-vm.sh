#!/usr/bin/env bash
# Launch 1 GCE VM for EigenLayer rewards backfill
#
# Migrated 2026-05-08 (Tab 11) from
# `e2e-testing/scripts/defi/launch_eigenlayer_rewards_vm.sh` per the
...
#
# Fetches all RewardsClaimed events from the EigenLayer RewardsCoordinator
# contract (0x7750d328b314EfFa365A0402CcfD489B80B0adda) from inception
# (2024-08-06) to present, enriches with USD prices from DefiLlama,
# and writes daily parquet files to GCS.
#
...
TARBALL_NAME="eigenlayer_rewards_codebase.tar.gz"
VM_NAME="mtds-eigenlayer-rewards-backfill"
...
echo "  Data:   EigenLayer RewardsClaimed events + DefiLlama prices/TVL"

# Pre-fetch Alchemy API key from Secret Manager (local machine has access)
...
if [[ -z "$ALCHEMY_KEY" ]]; then
  echo "WARNING: Could not fetch alchemy-api-key from Secret Manager"
...
#!/bin/bash
set -euo pipefail
export WORK_DIR=/tmp/eigenlayer_rewards
export HOME=/root
export PATH="/root/.local/bin:\$PATH"

exec > >(tee /var/log/eigenlayer-rewards.log) 2>&1
...
(while true; do
  sleep 60
  gsutil -q cp /var/log/eigenlayer-rewards.log \
...
echo ""
echo "=== Starting EigenLayer rewards backfill ==="
echo "  Range: ${START_DATE} → ${END_DATE}"
date

cd market-tick-data-service
python3 -m market_tick_data_service \
  --operation collect-eigenlayer-rewards \
  --mode batch \
...
echo ""
echo "SSH + tail log:"
echo "  gcloud compute ssh ${VM_NAME} --zone=${ZONE} --project=${PROJECT_ID} -- tail -f /var/log/eigenlayer-rewards.log"
echo ""
echo "Stream logs from GCS:"
echo "  gsutil cat ${GCS_STAGING}/logs/${VM_NAME}_final.log | tail -100"
echo ""
echo "Check results:"
echo "  gsutil ls ${TICK_BUCKET}/raw_tick_data/by_date/day=2024-08-15/category=defi/venue=EIGENLAYER-ETHEREUM/"
echo "  gsutil ls ${TICK_BUCKET}/raw_tick_data/by_date/ | grep EIGENLAYER | wc -l"
echo "============================================================"
