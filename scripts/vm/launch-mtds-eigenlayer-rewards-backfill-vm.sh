...
# Lifecycle: permanent
# Delete-when: NA
# Launch GCE VM for EigenLayer rewards backfill (RewardsClaimed events + DefiLlama prices)
#
...
#
# ~195K claim events across ~600 days from inception (2024-08-06).
# Takes ~30-60 min on e2-standard-4.
#
# Usage:
#   bash launch-mtds-eigenlayer-rewards-backfill-vm.sh
#   bash launch-mtds-eigenlayer-rewards-backfill-vm.sh --start 2024-08-06 --end 2026-01-01
#   bash launch-mtds-eigenlayer-rewards-backfill-vm.sh --dry-run
#   bash launch-mtds-eigenlayer-rewards-backfill-vm.sh --env staging
set -euo pipefail
...
START_DATE="${START_DATE:-2024-08-06}"
...
echo "============================================================"
echo "EigenLayer Rewards Backfill VM Launcher (Pattern A)"
echo "  VM:        ${VM_NAME}"
...
if ! $FORCE; then
  EXISTING="$(gcloud compute instances list \
    --filter="name~\"^mtds-eigenlayer-rewards-\" AND status=RUNNING" \
    --zones="${ZONE}" \
    --project="${PROJECT_ID}" \
    --format='value(name)' 2>/dev/null | head -1 || true)"
  if [[ -n "$EXISTING" ]]; then
    echo "WARN: EigenLayer rewards VM already running: ${EXISTING}" >&2
...
if $DRY_RUN; then
  ...
  echo "  VM_TASK=defi-backfill  VM_OPERATION=collect-eigenlayer-rewards"
  exit 0
fi
...
METADATA="${METADATA},VM_OPERATION=collect-eigenlayer-rewards"
...
gcloud compute instances create "${VM_NAME}" \
  ...
  --labels="purpose=mtds-eigenlayer-rewards-backfill,env=${DEPLOYMENT_ENV}",managed-by=deployment-service \
  --metadata="${METADATA}"
