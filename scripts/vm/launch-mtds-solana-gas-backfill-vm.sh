...
# Lifecycle: permanent
# Delete-when: NA
# Launch GCE VM for Solana gas fee backfill (Alchemy Solana archival RPC)
#
...
# Handler: VM_TASK=solana-gas-backfill added to setup-data-pipeline-vm.sh same commit.
#
# Solana gas fee data (getBlock + full transactions) from ~2021-01-01 onwards.
# Alchemy Solana archival RPC required — fetches key from Secret Manager at VM boot.
#
# Usage:
#   bash launch-mtds-solana-gas-backfill-vm.sh
#   bash launch-mtds-solana-gas-backfill-vm.sh --start 2024-01-01 --end 2026-01-01
#   bash launch-mtds-solana-gas-backfill-vm.sh --dry-run
#   bash launch-mtds-solana-gas-backfill-vm.sh --env staging
set -euo pipefail
...
echo "============================================================"
echo "Solana Gas Fee Backfill VM Launcher (Pattern A)"
echo "  VM:        ${VM_NAME}"
...
if ! $FORCE; then
  ...
  if [[ -n "$EXISTING" ]]; then
    echo "WARN: Solana gas VM already running: ${EXISTING}" >&2
...
if $DRY_RUN; then
  ...
  echo "  VM_TASK=solana-gas-backfill"
  exit 0
fi

METADATA="startup-script-url=gs://${CODE_BUCKET}/vm/setup-data-pipeline-vm.sh"
METADATA="${METADATA},VM_TASK=solana-gas-backfill"
