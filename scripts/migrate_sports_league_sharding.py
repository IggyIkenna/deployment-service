...
def _resolve_af_league_id(af_id: object) -> str:
    ...
    try:
        numeric_id = int(af_id)  # pyright: ignore[reportArgumentType]  # Optional[str] passed where str expected; None excluded by caller guard
...
def _migrate_blob(
    ...
    """
    try:
        raw_data = storage_client.download_bytes(bucket, blob_path)  # pyright: ignore[reportOptionalMemberAccess]  # storage_client is narrowed to non-None by caller guard; union is Optional[StorageClient]
        ...
        df = extractor(df)  # pyright: ignore[reportOperatorIssue]  # value may be float|None from dict.get; None excluded by conditional
        ...
        for league_id, league_df in df.groupby("league_id"):
            ...
            if dry_run:
                ...
                storage_client.upload_bytes(  # pyright: ignore[reportOptionalMemberAccess]  # storage_client is narrowed to non-None by caller guard; union is Optional[StorageClient]
