...
def _count_rows(storage_client: object, bucket: str, blob_path: str) -> int:
    try:
        raw = storage_client.download_bytes(bucket, blob_path)  # pyright: ignore[reportOptionalMemberAccess]  # storage_client is narrowed to non-None by caller guard; union is Optional[StorageClient]
