# Terraform configuration for features-multi-timeframe-service
...
# Runs after features-delta-one-service (scheduled 12:00 PM UTC, FDS at 11:00 AM).
  ...
  steps:
    ...
    - check_features:
        switch:
          ...
          - condition: $${map.get(features_status.body, "failedCount") != null and map.get(features_status.body, "failedCount") > 0}
            raise: "features-multi-timeframe failed"
    ...
    - return_success:
        return:
          ...
          message: "features-multi-timeframe-service completed"
YAML
  ...
  steps:
    - init:
        assign:
          ...
          - categories: $${default(map.get(args, "categories"), ["CEFI"])}
    ...
    - return_success:
        return:
          ...
          end_date: $${end_date}
          execution: $${execution_name}
          message: "features-multi-timeframe-service backfill completed"
YAML
}
...
module "daily_job" {
  ...
  gcs_volumes = [
    ...
    { name = "features-multi-timeframe-cefi", bucket = "features-multi-timeframe-cefi-${var.project_id}", read_only = false },
    { name = "features-multi-timeframe-defi", bucket = "features-multi-timeframe-defi-${var.project_id}", read_only = false },
    { name = "features-multi-timeframe-tradfi", bucket = "features-multi-timeframe-tradfi-${var.project_id}", read_only = false },
  ]
...
module "backfill_workflow" {
  ...
  project_id            = var.project_id
  region                = var.region
  description           = "Historical backfill workflow for features-multi-timeframe-service"
  service_account_email = var.service_account_email
