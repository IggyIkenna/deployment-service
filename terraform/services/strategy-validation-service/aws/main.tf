# Terraform configuration for strategy-validation-service on AWS
...
terraform {
  ...
  backend "s3" {
    ...
    # key    = "services/strategy-validation-service/terraform.tfstate"
...
locals {
  ...
  workflow_definition = jsonencode({
    Comment = "Daily T+1 workflow for strategy-validation-service"
    StartAt = "ComputeDate"
    States = {
      ComputeDate = {
        Type = "Pass"
      ...
      SubmitBatchJob = {
        ...
        Parameters = {
          JobName       = "strategy-validation-service-daily"
          JobDefinition = module.daily_job.arn
          JobQueue      = var.job_queue_arn
          ContainerOverrides = {
            Command = [
              "--operation", "validate", "--mode", "batch",
              "--category", "CEFI",
...
module "daily_job" {
  ...
  environment_variables = {
    ...
    VALIDATION_BUCKET          = var.validation_bucket
  }
