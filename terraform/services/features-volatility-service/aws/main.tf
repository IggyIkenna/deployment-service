# Terraform configuration for features-volatility-service on AWS
...
terraform {
  ...
  backend "s3" {
    ...
    # key    = "services/features-volatility-service/terraform.tfstate"
...
locals {
  ...
  workflow_definition = jsonencode({
    Comment = "Daily T+1 workflow for features-volatility-service"
    StartAt = "ComputeDate"
    States = {
      ComputeDate = {
        Type = "Pass"
      ...
      SubmitBatchJob = {
        ...
        Parameters = {
          JobName       = "features-volatility-service-daily"
...
# AWS Batch Job for features-volatility-service
module "daily_job" {
...
module "daily_workflow" {
  ...
  # Schedule at 10:30 AM UTC daily (parallel with features-delta-one)
