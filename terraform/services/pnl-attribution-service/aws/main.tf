# Terraform configuration for pnl-attribution-service on AWS
...
terraform {
  ...
  backend "s3" {
    ...
    # key    = "services/pnl-attribution-service/terraform.tfstate"
...
locals {
  ...
  workflow_definition = jsonencode({
    Comment = "Daily T+1 workflow for pnl-attribution-service"
    StartAt = "ComputeDate"
    States = {
      ComputeDate = {
        Type = "Pass"
      ...
      SubmitBatchJob = {
        ...
        Parameters = {
          JobName       = "pnl-attribution-service-daily"
          JobDefinition = module.daily_job.arn
          JobQueue      = var.job_queue_arn
          ContainerOverrides = {
            Command = [
              "--operation", "attribute", "--mode", "batch",
              "--asset-group", "CEFI",
...
module "daily_job" {
  ...
  environment_variables = {
    ...
    PNL_ATTRIBUTION_BUCKET  = var.pnl_attribution_bucket
  }
