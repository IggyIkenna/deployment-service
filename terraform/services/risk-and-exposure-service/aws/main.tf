# Terraform configuration for risk-and-exposure-service on AWS
...
terraform {
  ...
  backend "s3" {
    ...
    # key    = "services/risk-and-exposure-service/terraform.tfstate"
...
locals {
  ...
  workflow_definition = jsonencode({
    Comment = "Daily T+1 workflow for risk-and-exposure-service"
    StartAt = "ComputeDate"
    States = {
      ComputeDate = {
        Type = "Pass"
      ...
      SubmitBatchJob = {
        ...
        Parameters = {
          JobName       = "risk-and-exposure-service-daily"
...
module "daily_job" {
  ...
  environment_variables = {
    ...
    RISK_METRICS_BUCKET      = var.risk_metrics_bucket
  }
