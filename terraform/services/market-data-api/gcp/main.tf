# Terraform configuration for market-data-api
# HTTP API exposing normalized market data for strategy and ML services
# Runs as a periodic Cloud Run Job (every 15 minutes) to refresh cached OHLCV data
...
locals {
  # Live workflow: refresh cached market data every 15 min
...
main:
  ...
  steps:
    ...
    # Run market data refresh — single cycle for all categories
    - run_refresh:
        call: http.post
        args:
    ...
    - check_refresh:
        switch:
          ...
          - condition: $${map.get(refresh_status.body, "failedCount") != null and map.get(refresh_status.body, "failedCount") > 0}
            raise: "market-data-api refresh failed"
    ...
    - return_success:
        return:
          ...
          message: "market-data-api cache refresh completed"
YAML
}

# Cloud Run Job for market-data-api cache refresh
module "refresh_job" {
  ...
  gcs_volumes = [
    ...
    { name = "market-data-api-cache", bucket = "market-data-api-cache-${var.project_id}", read_only = false },
  ]
...
module "live_workflow" {
  ...
  project_id            = var.project_id
  region                = var.region
  description           = "Live workflow for market-data-api cache refresh (every 15 min)"
  service_account_email = var.service_account_email
