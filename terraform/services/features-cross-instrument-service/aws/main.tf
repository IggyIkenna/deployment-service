# Terraform configuration for features-cross-instrument-service on AWS
...
terraform {
  ...
  backend "s3" {
    ...
    # key    = "services/features-cross-instrument-service/terraform.tfstate"
...
locals {
  ...
  workflow_definition = jsonencode({
    Comment = "Daily T+1 workflow for features-cross-instrument-service"
    StartAt = "ComputeDate"
    States = {
      ComputeDate = {
        Type = "Pass"
      ...
      SubmitBatchJob = {
        ...
        Parameters = {
          JobName       = "features-cross-instrument-service-daily"
