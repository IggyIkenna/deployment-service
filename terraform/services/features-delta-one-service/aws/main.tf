# Terraform configuration for features-delta-one-service on AWS
...
terraform {
  ...
  backend "s3" {
    ...
    # key    = "services/features-delta-one-service/terraform.tfstate"
...
locals {
  ...
  workflow_definition = jsonencode({
    Comment = "Daily T+1 workflow for features-delta-one-service"
    StartAt = "ComputeDate"
    States = {
      ComputeDate = {
        Type = "Pass"
      ...
      SubmitBatchJob = {
        ...
        Parameters = {
          JobName       = "features-delta-one-service-daily"
...
# AWS Batch Job for features-delta-one-service
module "daily_job" {
...
module "daily_workflow" {
  ...
  # Schedule at 10:30 AM UTC daily (after market-data-processing)
