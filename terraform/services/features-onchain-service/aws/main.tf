# Terraform configuration for features-onchain-service on AWS
...
terraform {
  ...
  backend "s3" {
    ...
    # key    = "services/features-onchain-service/terraform.tfstate"
...
locals {
  ...
  workflow_definition = jsonencode({
    Comment = "Daily T+1 workflow for features-onchain-service"
    StartAt = "ComputeDate"
    States = {
      ComputeDate = {
        Type = "Pass"
      ...
      SubmitBatchJob = {
        ...
        Parameters = {
          JobName       = "features-onchain-service-daily"
...
# AWS Batch Job for features-onchain-service
module "daily_job" {
...
module "daily_workflow" {
  ...
  # Schedule at 10:30 AM UTC daily (parallel with other features)
