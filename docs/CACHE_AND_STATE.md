...
**Post-2026-05-06 additions** — manifest concurrency: per-VM shards under `_index/per_vm/{vm_name}.parquet` merged by `manifest_consolidator` daemon into canonical `_index/availability_index.parquet`. Reader fallback merges per-VM shards when canonical blob older than `MANIFEST_CONSOLIDATED_STALENESS_SEC` (default 120s).
